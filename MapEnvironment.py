...
class MapEnvironment(object):
    ...
    def state_validity_checker(self, config):
        ...
        """
        # TODO: YOUR IMPLEMENTATION HERE
        config = config.astype(np.int32)
        if config[0, 0] < 0 or config[0, 0] > self.ylimit[1] or config[1, 0] < 0 or config[1, 0] > self.xlimit[1]:
            return False
        for i in range(config.shape[1]):
            if self.map[config[0, i], config[1, i]] == 1:
                return False
        return True
    ...
    def h(self, config):
        ...
        """
        # TODO: YOUR IMPLEMENTATION HERE
        dist_x = np.abs(config[0] - self.goal[0])
        dist_y = np.abs(config[1] - self.goal[1])
